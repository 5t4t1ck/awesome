...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({"develop","www","down","vbox"}, s, layouts[1])
end
-- }}}
...
---------------------------------------------------------------------------------
--Separador
separador = wibox.widget.textbox(" ")
...
'<span background="#006767" font="sans 8"> <span font="sans 8" color="#EEEEEE">${/ used_gb}/${/ avail_gb} GB </span></span>',
800)
...
for s = 1, screen.count() do
    ...
    mywiboxinferior[s] = awful.wibox({ position = "bottom", screen = s })
...
--	Widgets bottom wibox left
--    local left_layoutinferior = wibox.layout.fixed.horizontal()
--	
--	left_layoutinferior:add(batwidget)

    -- Widgets bottom wibox right
  --  local right_layoutinferior = wibox.layout.fixed.horizontal()
	 
--	right_layoutinferior:add(cpulabel)

  ---  mywiboxinferior[s]:set_widget(layout)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ }, "Print", function () awful.util.spawn("scrot -e 'mv $f ~/screenshots/%Y-%m-%d-%T-screenshot.png 2>/dev/null'") end),
